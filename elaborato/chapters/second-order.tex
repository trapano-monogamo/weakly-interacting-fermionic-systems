...
\begin{theorem}
	...
	$\lim_{\vert k\vert \to \infty} \epsilon_k / \vert k\vert > 0$, the above expression
...
\begin{proof}
	...
	conclude that $\vert\hat{V}(k)\vert \leq C_1/\vert k\vert^m$ for some $C_1>0$, which means
	...
	infinity. As for the energy terms, we are assuming that $\epsilon_k\geq C_2\vert k\vert$
	for all $k$ outside of the ball of radius $K$ for some $K,C_2>0$. The last factor to
	discuss is the Fermi-Dirac distribution, but this has an explicit form: an exponential
	decay in $\epsilon_k$, which is bounded at least by an explonential decay in $\vert k\vert$.
...
\begin{align*}
	...
	\end{cases}
\end{align*}
which implies the following: \textcolor{red}{YOUR MOM}
\begin{align*}
\end{align*}
