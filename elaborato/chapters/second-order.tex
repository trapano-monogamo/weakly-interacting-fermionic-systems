...
\begin{lemma}
	...
	\begin{equation}
		...
		= \delta_{k,k'} n_{\text{F}}(\epsilon_k) e^{-(\tau-\tau')\epsilon_{k}}
	\end{equation}
	where $n_\text{F}(\epsilon) = (e^{\beta\epsilon}+1)^{-1}$ is the Fermi-Dirac distribution.
\end{lemma}
\begin{proof}
...
The relevant limits we are going to take, and for which we need to provide proof of
convergence, are respectively: $\Lambda\to\infty$ to remove the UV cutoff, $\beta\to\infty$ to
get to the ground state energy, and $L^3\to\infty$ for the thermodynamic limit
(\textcolor{red}{check that you actually defined $L$ to be the sidelength of the volume occupied by the system}).
(\textcolor{red}{check the appropriate factor $2\pi$ for $\mathbb{Z}^3$}).
...
\begin{proof}
	...
	the respective sum over $\mathbb{Z}^3$. Because the general terms of each of the above
	...
	Now we can show that each term converges absolutely. Indeed $\sum_{k\in\mathbb{Z}^3}n_k$
	converges if, and only if, the series $\sum_{\ell\in\mathbb{N}} \ell^2 e^{-\beta\epsilon_\ell}$
	...
	\end{align*}
	This shows that indeed the first order has, as a limit in $\Lambda\to\infty$, an
	...
	\end{align*}
	which, once more turning sums over $\mathbb{Z}^3$ into normal series with an additional square
