...
\begin{proposition}
	$A$ is a symmetric operator if and only if $A^*$ is an extension of $A$, i.e. $A \subseteq A^*$.
\end{proposition}
\begin{proof} \textcolor{red}{????} \end{proof}

\begin{definition}[Compact, trace class and Hilbert-Schmidt operators]
	An operator $A$ is said to be \textit{compact} if $D(A) = \mathcal{H}$ and there are
	orthonormal bases $\{u_n\}_{n\in\mathbb{N}}$ and $\{v_n\}_{n\in\mathbb{N}}$ and a sequence
	$\{\lambda_n\}_{n\in\mathbb{N}}\subset\mathbb{C}$ with $\lim_n \lambda_n = 0$ such that
	\begin{align*}
...
We now present some important theorems for which we will not give the proof, as they go beyond
the scope of our work. Recall that the dual of a vector space is the set of all the
\textit{continuous} linear functionals on that space, where the topology that defines continuity
is once again the norm topology.

\textcolor{red}{MAYBE THESE THEOREMS COULD BE PUT IN AN APPENDIX?}

\begin{theorem}[Riesz's representation theorem]
	Given a linear map $F : \mathcal{H} \to \mathbb{C}$, $F\in\mathcal{H}^*$ if, and only if,
	there exists a unique $x_F\in\mathcal{H}$ such that $F(x) = \langle x_F, x \rangle$ for
	all $x\in\mathcal{H}$. Furthermore, $\Vert F\Vert_{\mathcal{H}^*} = \Vert x_F \Vert_{\mathcal{H}}$.
\end{theorem}
\begin{proof}
	\textcolor{red}{AAAAAAAHHHHHHHH.}
\end{proof}

\begin{theorem}[Spectral theorem for compact operators]
	Given a compact symmetric operator $A$, there exists an orthonormal basis
	$\{\phi_n\}_{n\in\mathbb{N}}$ for $\mathcal{H}$ and a sequence $\{\lambda_n\}_{n\in\mathbb{N}}$
	such that $\lim_n \lambda_n = 0$ and
	\begin{align*}
		A\psi = \sum_{n=1}^\infty \lambda_n \langle \phi_n, \psi \rangle \phi_n \quad \forall \psi\in\mathcal{H}.
	\end{align*}
\end{theorem}

The set $\{\lambda_n\}_n$ is the point spectrum of $A$ and $\{\phi_n\}_n$ are its eigenvectors,
which also form an orthonormal basis for $\mathcal{H}$.

The most important Hilbert space for us is the space of square integrable functions on $\mathbb{R}^n$,
i.e.
\begin{align*}
	L^2(\mathbb{R}^n) = \left\{ f : \mathbb{R}^n \to \mathbb{C}
	\;\Big| \left(\int_{\mathbb{R}^n} \dx x\, |f(x)|^2\right)^{1/2} < \infty \right\},
\end{align*}
whose elements are the wavefunctions of a physical system.
...
Assuming the observable $A$ is a \textcolor{red}{compact} symmetric operator, by the spectral theorem we
can write it as a series of projectors onto its eigenspaces. Let
...
\textcolor{red}{Add discussion about direct sum of hilbert spaces and prove that direct sums
and tensor products of hilbert spaces are still hilbert spaces?}
...
subspaces of a tensor product that correspond, as we will see, to the space of actual physical
states for bosonic and fermionic systems, and we will naturally be intersted in restricting
operators to those subspaces via projections.

\begin{definition}[Tensor product Hilbert space]
	Given $\mathcal{H} \text{ and } \mathcal{K}$ two Hilbert spaces, their \textit{tensor product}
	is a Hilbert space, denoted $\mathcal{H}\otimes\mathcal{K}$, together with a bilinear map
	\begin{align*}
		\mathcal{H}\times\mathcal{K} \ni (u,v) \mapsto u \otimes v \in \mathcal{H}\otimes\mathcal{K}
	\end{align*}
	such that the inner product satifies
	\begin{align*}
		\langle u_1 \otimes v_1, u_2 \otimes v_2 \rangle_{\mathcal{H}\otimes\mathcal{K}}
		= \langle u_1,u_2 \rangle_{\mathcal{H}} \langle v_1,v_2 \rangle_{\mathcal{K}}
	\end{align*}
	for all $u_1,u_2 \in \mathcal{H} \text{ and } v_1,v_2 \in \mathcal{K}$, and such that
	the space $\mathrm{span}\{ u \otimes v : u\in\mathcal{H}, v\in\mathcal{K} \}$ is dense
	in $\mathcal{H}\otimes\mathcal{K}$.
\end{definition}

\begin{remark}
	\textcolor{red}{One can show} that the tensor product between Hilbert spaces is unique up to
	isometric isomorphism (i.e. up to an isomorphism that preserves the inner product). Moreover,
	given orthonormal bases $\{u_i\}_{i\in I} \text{ for } \mathcal{H} \text{ and } \{v_j\}_{j\in J}
	\text{ for } \mathcal{K}$, the set $\{u_n\otimes v_m\}_{n,m}$ is a basis for the space
	$\mathcal{H}\otimes\mathcal{K}$, which can be naturally identified with the space
	$\ell^2(I \times J)$ where the inner product acts as
	\begin{align*}
		(u \otimes v)_{ij} = \langle u_i,u \rangle_{\mathcal{H}} \langle v_j,v \rangle_{\mathcal{K}}.
	\end{align*}
\end{remark}
\begin{remark}
	The space $L^2(\mathbb{R}^n)\otimes L^2(\mathbb{R}^m)$ can be naturally identified with
	the space $L^2(\mathbb{R}^{n+m})$, and given $f\in L^2(\mathbb{R}^n) \text{ and } g\in L^2(\mathbb{R}^m)$
	one has
	\begin{align*}
...
Given two operators $A$ on $\mathcal{H}$ and $B$ on $\mathcal{K}$ we can define an operator
$A \otimes B$ on $\mathcal{H}\otimes\mathcal{K}$ in a natural way. Its domain will be
\begin{align*}
	D(A \otimes B) = \{ \phi\otimes\psi : \phi\in D(A), \psi\in D(B) \},
\end{align*}
which is a dense subspace of the tensor product by construction, and on this subspace it will
act as
\begin{align*}
	A \otimes B (\phi\otimes\psi) = (A\phi)\otimes(B\psi).
\end{align*}

We can extend the definition of a tensor product to products of $N$ spaces in a very natural
way by substituting pure tensors with $N$-fold pure tensors and inner products with N-fold
products of the respective inner products. This way, the definition given is just the special
case $N=2$. Of special interest to us is the N-fold tensor product of $\mathcal{H}$ with
itself $\bigotimes^N\mathcal{H}$, on which we can define a natural action for the group of
permutations $S_N$. Indeed, we can define the unitary representation $U_\sigma$ of
$\sigma\in S_N$ on this space as
\begin{align*}
	U_\sigma (u_1 \otimes \cdots \otimes u_N) \coloneq u_{\sigma^{-1}(1)} \otimes \cdots \otimes u_{\sigma^{-1}(N)}.
\end{align*}

With this action, we define two important subspaces. Define the operators
\begin{align}
...
These operators are unitary by construction and \textcolor{red}{one can show} that they are
also projections.
...
Requiring that the particles be indistinguishable from one another means that both their
internal energy operators and the pair-wise interactions do not depend on the specific
particle considered. This means that
\begin{align*}
...
With these restrictions, it is clear that $H_N$ maps the spaces $\bigotimes_{\mathrm{sym}}^N\mathfrak{h}$
and $\bigwedge^N\mathfrak{h}$ into themselves, which means that we can restrict this N-body
Hamiltonian to either of the two subspaces and get a well defined operator (provided that a
suitable domain for the starting Hamiltonian is chosen).
\textcolor{red}{(Consider putting the procedure in Forte's book to justify fermionic and bosonic
statistics starting from exchange operator)}.
Because the two subspaces mentioned are, respectively, the spaces of states that are symmetric
and anti-symmetric under the exchange of any pair of particles, we have that the operators
\begin{align*}
	P_+(H_N)\quad\text{and}\quad P_-(H_N)
\end{align*}
are respectively the \textit{bosonic} and \textit{fermionic} N-body Hamiltonians for interacting
particles.
...
\begin{proposition}
	For all $f\in\mathfrak{h}$, $a(f) \text{ and } a^*(f)$ can be extended to the domain
	\begin{align*}
		\left\{ \Psi = \bigoplus_{N=0}^\infty\Psi_N : \sum_{N=0}^\infty N\Vert\Psi_N\Vert^2 < \infty \right\}.
	\end{align*}
\end{proposition}
\begin{proof}
	\textcolor{red}{???}
...
mechanics exactly by the antisymmetrization of fermionic states, indeed given two one-particle
...
\begin{theorem}[2nd quantization of 2-body operator]
	...
	$\mathfrak{h}\otimes\mathfrak{h}$ \textcolor{red}{such that (you didn't cover this
	in the section on tensor products)} $\mathrm{Ex}W\mathrm{Ex}=W$.
	Let $\{u_\alpha\}_{\alpha=1}^\infty$ be an orthonormal basis for $\mathfrak{h}$ such
	that $u_\alpha\otimes u_\beta \in D(W)$ for all $\alpha\text{ and }\beta$. Then as a
	quadratic form on the domain
	$\cup_{M=2}^\infty\bigoplus_{N=2}^M P_\pm(D(\sum_{1\leq i<j\leq N} W_{ij}))$ we have
	\begin{align*}
...
\begin{proposition}
	The operator $\gamma_\Psi$ is positive semi-definite \textcolor{red}{(btw you didn't introduce
	partial ordering of operators)} and trace class, with
	\begin{align*}
...
\end{proposition}
\begin{proof}
	\textcolor{red}{IT'S JUST EXPLICIT CALCULATION}.
\end{proof}
