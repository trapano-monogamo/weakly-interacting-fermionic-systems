...
\end{theorem}
\begin{proof}
	For every $F=f\oplus Jg\in\mathfrak{h}\oplus\mathfrak{h}^*$, we have $A_\pm(F) = a_\pm(f)
	+ a^*_\pm(g)$, and we can split the creation and annihilation part as $A_\pm(F)^- = a_\pm(f)$
	and $A_\pm(F)^+ = a^*_\pm(g)$. Calling now $A_\pm(F_i) = A_i$ we notice the following
	relations
	\begin{gather*}
		[A_i^-,A_j^-]_\mp = 0,\quad[A_i^+,A_j^+]_\mp = 0\\
		[A_i^-,A_j^+]_\mp = \langle\Psi,A_iA_j\Psi\rangle\\
		[A_i^-
	\end{gather*}
	\textcolor{red}{EMH... WHAT?}
\end{proof}
