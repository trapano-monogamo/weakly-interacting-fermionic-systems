...
We use the convention in which the inner product $\langle\cdot,\cdot\rangle$ is linear in the
second argument.
...
physical observable \textit{with discrete spectrum}, the result of an experiment is the
collapse of the system onto a state associated to a well-defined value of $A$.
When the spectrum is not discrete, the notion of a measureable quantity is best understood in
terms of projection-valued measures, which we will not cover in our work. A generic state
$\psi$ will be a superposition of such vectors, which we may call $\{\psi_i\}_{i\in\mathbb{N}}$,
and let us call $\{\lambda_i\}_{i\in\mathbb{N}}\subset\mathbb{R}$ the well-defined values of
$A$ associated to such vectors. The generic state $\psi$ then encodes the probability
amplitudes $\langle\psi_i,\psi_i\rangle$ for the system to collapse in each $\psi_i$, which
suggests that the statistical expectation for the measurement of $A$ would read something like
\begin{align*}
...
definitions and properties of special types of operators. A key role in the discussion of
quantum systems is played by the space of square integrable functions on $\mathbb{R}^n$ modulo
the equivalence relation $\sim$ that identifies functions that are equal almost everywhere:
\begin{align*}
...
\end{align*}
whose elements are called wave functions. Its relevance stands from the fact that, in
...
\begin{remark}
	We could introduce Gibbs states following the standard thermodynamical approach. We would
	define the entropy as $S(\langle\,\cdot\,\rangle) = -\sum_{n}p_n\log(p_n)$, then the free
	energy as $F(T) = \inf_{\langle\,\cdot\,\rangle}(\langle H\rangle - TS(\langle\,\cdot\,\rangle$),
...
\end{remark}
\begin{remark}
	A free particle with state space $L^2(\mathbb{R}^n)$ and governed by the Hamiltonian
	$H=-\Delta$ is stable but does not admit a ground state. However, this Hamiltonian does
	admit a ground state when restricting the state space to wave functions on a compact
	box with periodic boundary conditions. The stability of free particles is essential,
...
(but this procedure can be readily generalized to $N$-body interactions) to each pair $(i,j)$ of
distinct particles ($i\neq j$) we associate an operator $W_{ij}$ on a suitable domain. To lift
this operator, consider vectors $\phi_i\in\mathfrak{h}_i$ and identify $W_{ij}$ with the
...
\end{align*}
where $\psi_i\otimes\psi_j = W_{ij}\phi_i\otimes\phi_j$ (and express the image as
...
\begin{align}
	\label{indistinguishable-operators}
...
\begin{definition}[Symmetric and anti-symmetric tensor products]
	...
	is the subspace
	\begin{align*}
		\bigotimes_{\mathrm{sym}}^N\mathfrak{h} \coloneq P_+ \left( \bigotimes^N\mathfrak{h} \right) \subset \mathcal{H}_N
	\end{align*}
...
\begin{remark}
	...
	products between wave functions can be constructed as Slater determinants, i.e.
	\begin{align*}
...
With the conditions in Equation \eqref{indistinguishable-operators} that we impose when
introducing indistinguishability, it is clear that $H_N$ maps the spaces
...
\label{eq:creation-annihilation}
	&a(f)(f_1\otimes\cdots\otimes f_N) \coloneq \langle f,f_1\rangle_\mathfrak{h} f_2\otimes\cdots\otimes f_N,\\
	&a^*(f)(f_1\otimes\cdots\otimes f_N) \coloneq f\otimes f_1\otimes\cdots\otimes f_N.\nonumber
\end{align}
...
\begin{proof}
	...
	\begin{align*}
		...
		&= \langle f,\phi_0\rangle\langle\psi_1\otimes\cdots\otimes\psi_N,\phi_1\otimes\cdots\otimes\phi_N\rangle\\
		&= \langle f,\phi_0\rangle\prod_{j=1}^N\langle\psi_j,\phi_j\rangle,
	\end{align*}
	and similarly
	\begin{align*}
...
subspaces:
\begin{align}
	\label{bosonic-fermionic-operators}
	a_\pm(f) \coloneq \sqrt{N}\,a(f), \quad a^*_\pm(f) \coloneq \sqrt{N}\,P_\pm (a^*(f)) \quad \forall f\in\mathfrak{h}.
\end{align}
...
The normalization we used in Equation \eqref{bosonic-fermionic-operators} is chosen so that
the canonical (anti)commutation relations hold. Recall the definition of the commutator and
anticommutator between two operators defined on a common domain: $\{A,B\}\coloneq AB + BA$ and
$[A,B]\coloneq AB - BA$. The anticommutator may be denoted by $[\,\cdot,\cdot\,]_+$ while the
commutator by $[\,\cdot,\cdot\,]_-$, to accommodate for the more compact notation
$[\,\cdot,\cdot\,]_\mp$ to indicate either of the two.
...
\end{proposition}
\begin{proof}
	We prove the claim on $N$-fold pure tensors. Let $f,g,\phi_1,\ldots,\phi_N\in\mathfrak{h}$,
	then we may write the action of creation and annihilation as
	\begin{align*}
		a^*_\pm(f)P_\pm(\phi_1\otimes\cdots\otimes\phi_N)
		&= \sqrt{N+1}P_\pm(f\otimes P_\pm(\phi_1\otimes\cdots\otimes\phi_N))\\
		&= \sqrt{N+1}P_\pm(f\otimes\phi_1\otimes\cdots\otimes\phi_N);\\
		a_\pm(f)P_\pm(\phi_1\otimes\cdots\otimes\phi_N)
		&= \frac{\sqrt{N}}{N!}\sum_{\sigma\in S_N}(\pm)^\sigma\langle f,\phi_{\sigma(1)}\rangle\phi_{\sigma(2)}\otimes\cdots\otimes\phi_{\sigma(N)}\\
		&= \frac{1}{\sqrt{N}}\sum_{k=1}^N(\pm)^{k-1}\langle f,\phi_k\rangle P_\pm(\phi_1\otimes\cdots\otimes\widehat{\phi_k}\otimes\cdots\otimes\phi_N),
	\end{align*}
	where the hat symbol indicates omission. The products in $[\cdot,\cdot]_\mp$ are
	\begin{align*}
		a_\pm(f)a^*_\pm(g)P_\pm(\phi_1\otimes\cdots\otimes\phi_N)
		&= a_\pm(f)\sqrt{N+1}P_\pm(g\otimes\phi_1\otimes\cdots\otimes\phi_N)\\
		&= \langle f,g\rangle P_\pm(\phi_1\otimes\cdots\otimes\phi_N)\\
		&\quad+ \sum_{k=1}^N(\pm)^k\langle f,\phi_k\rangle P_\pm(g\otimes\phi_1\otimes\cdots\otimes\widehat{\phi_k}\otimes\cdots\otimes\phi_N);\\
		a^*_\pm(g)a_\pm(f)P_\pm(\phi_1\otimes\cdots\otimes\phi_N)
		&= a^*_\pm(g)\frac{1}{\sqrt{N}}\sum_{k=1}^N\langle f,\phi_k\rangle P_\pm(\phi_1\otimes\cdots\otimes\widehat{\phi_k}\otimes\cdots\otimes\phi_N)\\
		&= \sum_{k=1}^N(\pm)^{k-1}\langle f,\phi_k\rangle P_\pm(g\otimes\phi_1\otimes\cdots\otimes\widehat{\phi_k}\otimes\cdots\otimes\phi_N).
	\end{align*}
	We conclude that
	\begin{align*}
		[a_\pm(f),a^*_\pm(g)]_\mp P_\pm(\phi_1\otimes\cdots\otimes\phi_N)
		= \langle f,g\rangle P_\pm(\phi_1\otimes\cdots\otimes\phi_N).
	\end{align*}
\end{proof}
...
of creation/annihilation operators, and show that indeed observables in the grand
...
values of observables will inevitably reduce to expectation values of products of
...
\begin{proof}
	...
	\begin{align*}
		\mathrm{span}\bigg\{\{\psi\}\cup \big(P(\mathfrak{h})\cap\{\phi\}^\perp\big)\bigg\},
	\end{align*}
...
need to define the generalized creation and annihilation operators to see this.
...
\section{Bogolubov transformations and quasi-free states}
...
states in a Fock space for which Wick's theorem, which we will come to shortly, will hold in
...
\begin{proof}[Sketch of the Proof]
	...
	call $a_i = a_\pm(f_i)$ and $a^*_i = a^*_\pm(g_i)$. We first note the following identities.
