...
\cite{SolNotes}, \textcolor{red}{and we will specify the page number for the proofs they
provide. The proofs that do not have a page number are those that are not found in Solovej's
notes and that we came up with as useful exercises}.
...
\begin{remark}
	...
	\phi\rangle$ for all $\phi,\psi\in\mathcal{H}$, which is equivalent to saying that
...
\begin{remark}
	We could define Gibbs state following the standard thermodynamical approach. We would
	define the entropy, for example as $S(\langle\,\cdot\,\rangle) = -\sum_{n}p_n\log(p_n)$,
	then we would define the free energy as $F(T) = \inf_{\langle\,\cdot\,\rangle}
	(\langle H\rangle - TS$) and say that a Gibbs state, if it exists for finite temperature,
	as a minimizer for the free energy. We would find that such a minimizer is exactly the
	functional we wrote in the definition.
\end{remark}
...
On this space, the energy operator is naturally given by the sum of all possible N-body
Hamiltonians:
\begin{align*}
...
using the (anti-)symmetric subspaces: \textcolor{red}{IS THERE A TYPO IN BENEDIKTER'S ARTICLE?}
\begin{align*}
...
\end{align*}
This is because we want to choose a normalization such that canonical commutator from
quantum mechanics still holds, and that is exactly what we want to show now.
%This can be seen easily by writing the action of $a^*_\pm(f)$ on a pure tensor:
%\begin{align*}
%	a^*_\pm(f)\phi_1\otimes\cdots\phi_N
%	&= P_\pm a^*(f)\phi_1\otimes\cdots\otimes\phi_N\\
%	&= \frac{1}{(N+1)!}\sum_{\sigma\in S_N}\sum_{j=1}^{N+1}\sqrt{N+1}
%	\phi_{\sigma(1)}\otimes\cdots\otimes\mathop{f}_{(j\mathrm{-th})}\otimes\cdots\otimes\phi_{\sigma(N)}\\
%	&= \frac{1}{\sqrt{N+1}}\frac{1}{N!}\sum_{\sigma\in S_N}\sum_{j=1}^{N+1}
%	\phi_{\sigma(1)}\otimes\cdots\otimes\mathop{f}_{(j\mathrm{-th})}\otimes\cdots\otimes\phi_{\sigma(N)}\\
%\end{align*}
%but notice that the remaining factor of $N!$ is reabsorbed when restricting the action of
%$a^*_\pm(f)$ to the (anti)symmetric subspace.
