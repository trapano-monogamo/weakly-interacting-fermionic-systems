...
combinations of state vectors are just probabilistic superpositions. We will give the
necessary definitions regarding a particular class of vector spaces, even though many concepts
come from much more general settings.
...
A note on the choice of notation. Usually a vector is just indicated by a letter that doesn't
hold additional meaning, but there are situations in quantum mechanics where it is clearer to
use, for example, indices to enumerate a specific family of vectors, and to avoid ambiguity
it can be convenient to employ Dirac's ``bra-ket" notation. In this notation, a vector would
be denoted by $|\psi\rangle\in\mathcal{H}$, a linear functional as $\langle\psi|\in\mathcal{H}^*$,
and the inner product (with respect to a symmetric operator, which we haven't yet introduced)
as $\langle\psi,A\phi\rangle=\langle\psi|A|\phi\rangle$. It will be clear when this notation
is used and why it can help avoid ambiguities, but for the most part we won't make use of it.
...
combinations. That is, the set $\mathcal{S}$ acts as a \textit{basis} in the sense that any
vector can be identified with a sequence of components $\{a_n:n\in\mathbb{N}\}$, with the
condition that the series $\sum_{n\in\mathbb{N}}a_n u_n$ converges (in the norm topology of
$\mathcal{H}$). For simplicity, even when writing just ``Hilbert space" we will always assume
that the space is separable.

\begin{lemma}
	If $X$ is a dense subspace of the Hilbert space $\mathcal{H}$, it is always possible to
	choose a basis $\{u_n:n\in\mathbb{N}\}$ for $\mathcal{H}$ composed only of elements in $X$.
\end{lemma}
\begin{proof}
	\textcolor{red}{Is it easy?}
\end{proof}
...
\begin{definition}[Symmetric operators and observables]
	An operator $A$ is said to be \textit{symmetric} if
	$\langle\psi,A\phi\rangle = \langle A\psi,\phi\rangle$ for all $\phi,\psi\in D(A)$.
	An \textit{observable} is a physical quantity represented by a symmetric operator on some
	Hilbert space.
\end{definition}
...
\begin{remark}
	...
	i.e. that observables must have real eigenvalues.
\end{remark}
...
\begin{definition}[Bounded operator]
	An densely defined operator $A$ is said to be \textit{bounded} if it is continuous (with
	respect to the norm topology), which, by linearity, is equivalent to the condition
	\begin{align*}
...
basic objects used here. Before moving onto the discussion of how these objects come into play
in the treatment of our physical system, we introduce the most important Hilbert space for us:
the space of square integrable functions on $\mathbb{R}^n$
\begin{align*}
...
There is a bijection between the set of mixed states (while pure states are just a special
case) and the set of quantum states (i.e. a subset of the dual of $\mathfrak{B}(\mathcal{H})$).
The states in which we are interested in are thermal equilibrium states, which we know from
statistical mechanics populate the energy spectrum of the system according to Boltzmann's
distribution $e^{-\beta\epsilon}/Z$, and with the aforementioned bijection we can just
identify these states \textit{at a given temperature} with mixtures $\{(e^{-\beta\epsilon_j}/Z
,\psi_j)\}$ if $\psi_j$ are energy eigenstates. To make this precise we need to properly
define the foundamental thermodynamic quantities required, and in particular we want to
discuss the main focus or our work: the free energy, which we will actually define starting
from the known relation $F=U-TS$.

\begin{definition}[Von Neumann entropy]
	In the same notation used so far (and with $I = \mathbb{N}$), the \textit{Von Neumann
	entropy} of a state $\langle\cdot\rangle_\rho$ is defined as
	\begin{align*}
...
\begin{definition}[Stability and ground state]
	...
	\end{align*}
	A \textit{ground state}, \textbf{if it exists}, is a (normalized) minimizer for the energy
	expectation, i.e. a unit vector $\psi_0\in D(H)$ such that
	\begin{align*}
...
\begin{definition}[Free energy]
	...
	variation, then that state is called a \textit{Gibbs state at (inverse) temperature
	$\beta$}. We will denote Gibbs states by $\langle\cdot\rangle_\beta$, despite the slight
	ambiguity in the notation, and we will also specify the Hamiltonian in exam when needed
	with $\langle\cdot\rangle_{\beta,H}$.
\end{definition}

\begin{remark}
	A system composed of a single free particle, governed by $H=p^2$ is stable but it does not
	admit a ground state, and $F(T) = -\infty$ for all $T>0$.
\end{remark}
...
quantization. Allowing $N$ to vary leads to the definition of \textit{grand canonical picture}.
We shall discuss the state space for a system in the grand canonical picture, which we can
define quite easily recalling the results mentioned in Appendix \ref{appendix:direct-sum-tensor-product}.
