...
\begin{frame}[t]
	...
	\begin{columns}
		\begin{column}{.6\textwidth}
			\begin{itemize}
				\item<3-> Fermi energy $\epsilon(k_{\mathrm{F}})$ grows as
					$\mathcal{O}(N^{2/3})$\\
					$\implies$ \textbf{kinetic energy} grows as $\mathcal{O}(N^{5/3})$;
				\vfill\item<4-> number of interactions is $N(N-1)/2$\\
	...
	\only<6>{
		\begin{tikzpicture}[remember picture, overlay]
			\draw[red, thick] (semiclassical.north west) rectangle (semiclassical.south east);
			\draw[green, thick] (meanfield.north west) rectangle (meanfield.south east);
			\node[draw=red, very thick, fill=red!30, above=.5em of semiclassical, xshift=-3.5em, rounded corners]
				{semiclassical parameter};
			\node[draw=green, very thick, fill=green!30, above=.5em of meanfield, xshift=5em, rounded corners]
...
\begin{frame}
	...
	[\href{https://link.springer.com/article/10.1007/s00220-019-03654-7}{Hainzl-Porta-Rexze, 2019}]
	and then rigorous bosonization methods
	[\href{https://link.springer.com/article/10.1007/s00222-021-01041-5}{Benedikter-et-al, 2021}]:
	\begin{align*}
	...
	\begin{block}<4->{Theorem: leading order contribution}
		As $N\to\infty$ we have $E^{\mathrm{gs}}_N = E^{\mathrm{HF}}_N + o(1)$, and
		\begin{align*}
...
\begin{frame}
	...
	\uncover<2->{
		where
		\begin{itemize}
			\item $\mu$ is the chemical potential fixed by $N$;
			\item $V$ depends only on distances: $V(x_i-x_j)=V(\vert x_i-x_j\vert)$;
			\item $\hat{V}$ has compact support.
		\end{itemize}
	}
	\vfill
	\uncover<3-4>{
...
\begin{frame}
	...
	\begin{gather*}
		...
		\uncover<3->{
			...
			\begin{tikzpicture}[scale=.6, baseline=(current bounding box.center)]
				...
				\node (c) at (-1,-1) {} node[scale=0.5] at (-0.9,-0.5) {$q$};
				\node (d) at ( 1,-1) {} node[scale=0.5] at ( 0.9,-0.5) {$p$};
				\begin{scope}[very thick, decoration={
						markings,
	...
	\uncover<4->{
		...
		\begin{align*}
			...
			&= \lim_{\beta\to\infty}\lambda\left(\begin{tikzpicture}[scale=.3, baseline=(current bounding box.center)]
